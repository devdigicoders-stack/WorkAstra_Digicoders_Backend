...
export const generatePayroll = async (req, res) => {
    try {
        ...
        const employees = await User.find(userFilter).select("_id workShift companyId joiningDate").lean();
        ...
        for (const emp of employees) {
            try {
                // Skip if employee hasn't joined yet in this month
                if (emp.joiningDate) {
                    const joinMonth = new Date(emp.joiningDate).toISOString().slice(0, 7);
                    if (joinMonth > month) {
                        results.skipped++;
                        results.skippedReasons = results.skippedReasons || [];
                        results.skippedReasons.push({ userId: emp._id, reason: "Employee joined after this month" });
                        continue;
                    }
                }
                ...
                if (existing) { 
                    if (existing.status === "paid") {
                        ...
                        results.skipped++; 
                        results.skippedReasons = results.skippedReasons || [];
                        results.skippedReasons.push({ userId: emp._id, reason: "Payroll already paid" });
                        continue; 
                    } else {
                        // If draft or approved, delete the existing one to regenerate
                ...
                // METHOD 1: Calendar Days (30/31) — Industry Standard
                // Per day salary divided by total calendar days in the month (not working days)
                const [yr, mn] = month.split("-").map(Number);
                const calendarDays = new Date(yr, mn, 0).getDate(); // 28/29/30/31

                // Compute salary components
                const grossForCalc = structure.grossEarnings > 0 ? structure.grossEarnings : structure.basic;
                const perDaySalary = grossForCalc / calendarDays;  // ÷ 30 or 31
                
                let lopDeduction = parseFloat((lopDays * perDaySalary).toFixed(2));

                // === PENALTY DATE LOGIC ===
                // Fetch penalty dates for this month
                const penaltyDates = await PenaltyDate.find({
                    date: { $regex: `^${month}` },
                    $or: [{ companyId: emp.companyId }, { companyId: null }]
                }).lean();

                let penaltyDeduction = 0;
                for (const pd of penaltyDates) {
                    // Check if employee was absent (no attendance) on this penalty date
                    const attOnDate = await Attendance.findOne({ userId: emp._id, date: pd.date }).lean();
                    const wasAbsent = !attOnDate || attOnDate.status === "absent";
                    if (!wasAbsent) continue; // Present ya half-day — no penalty

                    // Check if employee had an approved PAID leave on this date
                    const hasPaidLeave = approvedLeaves.some(l => {
                        if (!l.leaveTypeId?.isPaid) return false;
                        return l.fromDate <= pd.date && l.toDate >= pd.date;
                    });
                    if (hasPaidLeave) continue; // Paid leave approved — no penalty

                    // Apply penalty: (multiplier - 1) extra days deducted (1 already counted in LOP)
                    const extraDays = pd.penaltyMultiplier - 1;
                    penaltyDeduction += parseFloat((extraDays * perDaySalary).toFixed(2));
                }

                if (penaltyDeduction > 0) {
                    lopDeduction = parseFloat((lopDeduction + penaltyDeduction).toFixed(2));
                }
                // === END PENALTY DATE LOGIC ===
